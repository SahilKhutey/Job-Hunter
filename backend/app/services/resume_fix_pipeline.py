...
async def auto_fix_resume_pipeline(resume_text: str, resume_json: dict, job_desc: str):
    """
    Orchestrates the 'Score -> Fix -> Re-score -> Diff' pipeline (Async).
    """
    try:
        # 1. Initial Assessment
        original_assessment = await score_resume(resume_text, resume_json, job_desc)
        
        # 2. Execute Improvements
        fixed_json = await fix_resume_ai(
            resume_json, 
            job_desc, 
        ...
        new_assessment = await score_resume(fixed_text, fixed_json, job_desc)
