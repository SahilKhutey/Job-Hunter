...
async def score_resume(resume_text: str, resume_json: dict, job_desc: str):
    """
    Orchestrates the multi-layered scoring of a resume against a job description (Async).
    """
    ...
    sem_score = await semantic_score(resume_text, job_desc)
    ...
    feedback = await generate_resume_feedback(resume_text, job_desc, missing)
